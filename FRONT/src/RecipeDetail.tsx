...
const RecipeDetail = () => {
  ...
  useEffect(() => {
    const fetchRecipe = async () => {
      try {
        const response = await fetch(`http://localhost:8083/api/recipes/id/${id}`);
        ...
        setError("Une erreur est survenue lors de la récupération de la recette.");
  ...
  return (
    <div>
      {recipe ? (
        <>
          <h2>{recipe.title}</h2>
          <p><strong>Temps de préparation :</strong> {recipe.preparation_time}</p>
          <p><strong>Difficulté :</strong> {recipe.difficulty}</p>
          <p><strong>Coût :</strong> {recipe.cost}</p>
          <p><strong>Nombre de personnes :</strong> {recipe.people_number}</p>
